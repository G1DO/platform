# Design — Self-Healing Kubernetes Platform for Ahoy
...
Ahoy is a real DTC + B2B sauce and condiment business in MENA (FastAPI + Next.js + Postgres + Redis + Celery, currently on a single AWS EC2 instance with CloudWatch-only observability). The platform described here is the infrastructure Ahoy would need at projected post-Series-A scale: ~25 engineers, six MENA cities, ~5k orders/day at peak, B2B onboarding underway. See [problem-statement.md](problem-statement.md) for the full context.

The triggering event is a real near-miss: a Paymob webhook backlog left ~200 orders in `PENDING` for 90 minutes during a flash promo. At 3× projected volume, that incident becomes a multi-hour revenue-and-refund event.
...
- Replacing managed Postgres (RDS stays).
- Cost optimization as a primary goal (right-sizing is in scope; dedicated FinOps is not).
- Service mesh (mTLS via cert-manager + NetworkPolicies cover the threat model at current service count).

## 3. Glossary

- **SLI** — Service Level Indicator. A measurable quantity (e.g., "fraction of `POST /orders` returning 2xx in <400 ms").
...
The platform replaces Ahoy's current single-EC2, push-deploy, log-only setup with a Kubernetes-native control plane built in six iterative phases. The end state has three trust zones inside the cluster and explicit boundaries to managed services and to the supply chain.
...
- **The Ahoy workload namespace** runs FastAPI, two Next.js apps, Celery workers, and an in-cluster Redis. Postgres remains on managed RDS.
...
- **Outside the cluster**, GitHub hosts the app and config repos; GitHub Actions builds and signs images via Cosign keyless ([ADR-003](adr/003-image-signing-cosign-keyless.md)); ECR holds images; Vault holds secrets; PagerDuty pages on-call.
...
- **CUJ-1: Order placement.** Customer → Ingress → Next.js Storefront → FastAPI → (Redis hold, RDS persist, Paymob payment intent) → response.
- **CUJ-2: Payment-webhook-to-ledger.** Paymob → Ingress → FastAPI webhook handler → Redis queue → Celery worker → RDS ledger commit.
...
**Production:** AWS EKS, three availability zones in a single MENA region (matching the business footprint per [problem-statement non-goal #1](problem-statement.md#non-goals)). Worker nodes in a managed node group sized for projected peak with headroom.

**Namespace layout:**

| Namespace | Purpose | NetworkPolicy default |
|---|---|---|
| `ahoy` | Application workloads (backend, storefront, dashboard, celery, redis) | Deny ingress except from ingress namespace; deny egress except to RDS, Paymob, S3, observability |
...
| `ingress` | NGINX ingress controller (or Gateway API implementation) | Allow ingress from internet; egress to `ahoy` namespace only |
...
The application is Ahoy as it exists today, repackaged for Kubernetes:

| Workload | Replicas | Resources (initial) | Notes |
|---|---|---|---|
| `backend` (FastAPI) | 3 (HPA 3–10) | 250m CPU / 512 Mi RAM request, 1 CPU / 1 Gi limit | Stateless; primary CUJ-1 + CUJ-2 path |
| `storefront` (Next.js) | 2 (HPA 2–6) | 200m / 256 Mi req, 500m / 512 Mi limit | Stateless |
| `dashboard` (Next.js) | 2 | 200m / 256 Mi req, 500m / 512 Mi limit | Operator-facing; not SLO'd |
| `celery-worker` | 3 (HPA on queue depth) | 250m / 512 Mi req, 1 CPU / 1 Gi limit | Carries CUJ-2 reconciliation |
| `celery-beat` | 1 (no autoscale) | 100m / 128 Mi req | Singleton scheduler |
| `redis` | 1 (StatefulSet) | 200m / 512 Mi req, 500m / 1 Gi limit | In-cluster; queue + cache |

**Packaging:** Each app has its own Helm chart in the config repo (`/charts/<app>`). Image digests are pinned in `values.yaml`; CI bumps the digest via PR.
...
- **`ahoy-app`** — application source code, Dockerfiles, GHA workflows.
...
sequenceDiagram
    ...
    CI->>Reg: build & push image (by digest)
    CI->>Sig: cosign sign --keyless (GHA OIDC)
    Sig-->>Reg: signature + cert attached to image
    Sig-->>Sig: Rekor transparency log entry
    CI->>Cfg: open PR bumping image digest in values.yaml
...
**Intake:** OpenTelemetry Collector deployed as a DaemonSet (per-node receivers for app-local sidecar/agent traffic) plus a Deployment (gateway for ingress-side collection and processing). Apps emit OTLP. The collector applies a **PII scrubber** ([ADR-005 follow-up](adr/005-observability-lgtm-self-hosted.md#follow-up-work-this-commits-us-to), closing [threat-model.md §B5:I](threat-model.md)) — explicit deny-list for fields `email`, `phone`, `address`, `lat`, `lon`, `guest_customer_phone`, `guest_delivery_address` (the PII surface from [threat-model.md §A1](threat-model.md)).
...
- **CUJ-2 timeliness** — application-emitted `webhook_to_ledger_seconds` histogram, recorded inside the Celery worker at the moment the DB transaction commits. `histogram_quantile(0.999, ...) < 60` is the SLO.
...
**Trace correlation:** every log line includes `trace_id` and `span_id` (OTel SDK + structured logging). Grafana's "logs to traces" feature gives one-click correlation from an alert to the offending request.
...
- **Liveness** — restart pod if app process is stuck. Endpoint: `/health/live` (process up, no dependency check).
- **Readiness** — remove from Service endpoints if dependency unhealthy. Endpoint: `/health/ready` (checks DB connection pool, Redis, can-reach-Paymob if applicable).
- **Startup** — give slow-starting workloads time before liveness kicks in. Same `/health/live`, longer threshold.

**HPA:**

- `backend`: CPU 70%, min 3 / max 10.
- `storefront`: CPU 70%, min 2 / max 6.
- `celery-worker`: **custom metric** — Celery queue depth — min 3 / max 20. Closes [threat-model.md §B2:D](threat-model.md) (queue depth explosion under webhook flood).

**PDBs:** every app has a PDB with `minAvailable: 1` (single-replica safety) or `maxUnavailable: 1` (multi-replica). Argo Rollouts respects PDBs; node drains respect PDBs; chaos experiments respect PDBs.

**Argo Rollouts (Phase 5):**

- Canary strategy: 10% → analysis → 25% → analysis → 50% → analysis → 100%.
- Analysis queries Prometheus for the CUJ SLO indicators. Specifically: error rate on `POST /orders`, p99 latency, and `webhook_to_ledger_seconds` p99.9. Any regression beyond the SLO threshold during the canary window → automated rollback.
- Rollout is the **only** way workload images change in production. Direct manifest edits trigger Argo CD drift correction.

**Backup and restore:**

- **RDS Postgres** — Aurora-style continuous backup if Aurora is chosen, otherwise RDS automated backup (30-day retention) + manual pre-deploy snapshot (per current Ahoy `deploy.yml`, kept as-is). Snapshot restoration is rehearsed quarterly; the runbook is committed to the config repo.
...
- Every image signed at build time by Cosign keyless (GHA OIDC).
...
- `disallow-paymob-log-mode-in-prod` — closes top-risk #1.
...
- Short-lived rotation: 24-hour rotation on `PAYMOB_HMAC_SECRET` and `JWT SECRET_KEY` ([threat-model.md §A6](threat-model.md)).
- No long-lived AWS keys in CI ([threat-model.md §B8:I, §B8:E](threat-model.md)) — OIDC trust only.
- The `.env` files in current Ahoy are replaced by ExternalSecret resources reading from Vault.
...
- Append-only Paymob webhook audit table (application-level change, [threat-model.md §B2:R](threat-model.md)).
...
| Traffic spike on `POST /orders` | CPU > 70% on backend pods | HPA scales up (3 → 10 replicas) |
| Celery queue backlog | Queue depth metric | HPA scales `celery-worker` (3 → 20) |
...
| Image vulnerability scan finding (HIGH/CRITICAL) | ECR scan + nightly job | Ticket | runbooks/cve-response.md |
...
- Connection pooling: PgBouncer as a sidecar or DaemonSet (deferred decision; see §9 open questions).
- Backups: RDS automated, 30-day retention. Plus the existing pre-deploy manual snapshot pattern from Ahoy's current `deploy.yml`.
- **Role separation** — closes [threat-model.md §A3, §B6](threat-model.md): app role has `INSERT/SELECT/UPDATE` on specific tables but **no `DELETE`/`TRUNCATE` on `journal_lines` or `journal_entries`**; admin role separately managed with break-glass auditing. DB-level append-only enforcement via trigger on the ledger tables.
- Migration: Alembic, run by the Argo CD pre-sync hook (single execution per deploy).
...
- Used as Celery broker and as the order-placement hold cache; both uses are tolerant to short outages (Celery retries; order hold is opportunistic).
...
- **No multi-region failover.** A regional AWS event takes Ahoy down. The non-goal is deliberate at this scale ([problem-statement non-goal #1](problem-statement.md#non-goals)).
- **PII risk in telemetry intake.** OTel Collector's PII scrubber is allow-list-based on fields we know exist; novel PII added to an app emits to telemetry unless the scrubber is updated. Mitigated by code-review discipline and periodic audit, not architecturally closed.
...
|---|---|---|---|---|
| 1 | Backend pod crash | Liveness probe fail | kubelet restart; HPA may add replica if sustained | Single pod; no user impact if N≥2 |
| 2 | Backend dependency unhealthy (RDS unreachable) | Readiness probe fail | Pod removed from Service; HPA cannot help; CUJ-1 starts burning budget | All `POST /orders` 5xx until RDS returns |
...
| 5 | RDS failover (Multi-AZ) | AWS event + connection failure | Pods reconnect via DNS; ~30–60s of `POST /orders` errors | CUJ-1 ~30–60s burn |
| 6 | Paymob outage (gateway down) | Outbound 5xx from paymob_service | Backend returns explicit 502 with retry guidance; orders queue in `AWAITING_PAYMENT` | CUJ-1 affected only for `INSTANT` orders; CUJ-2 stalls until Paymob returns |
| 7 | Paymob webhook flood ([threat-model.md §B2:D](threat-model.md)) | Ingress rate-limit metric spike; Celery queue depth alarm | Ingress rate-limit clamps; Celery HPA scales up; circuit-breaker on handler returns 503 → Paymob retries | CUJ-2 burn during ramp |
| 8 | Forged Paymob webhook (HMAC compromise, [threat-model.md §B2:S](threat-model.md)) | Out-of-band Paymob reconciliation job detects mismatch | Manual investigation; admin disables ingress to webhook endpoint; rotate secret | Financial — bounded by reconciliation interval |
| 9 | Ledger drift (DB write outside app, [threat-model.md §A3](threat-model.md)) | Nightly reconciliation job vs Paymob settlement report | **Page** + freeze ledger writes pending investigation | Financial; integrity-of-record question |
...
| 14 | ECR unavailable | Image pull errors on new pods | kubelet image cache serves existing pods; new pods fail until ECR returns | New scale-up blocked |
...
4. **PgBouncer placement.** Sidecar (per-app) gives connection pooling closest to the workload but multiplies pooler count; cluster-wide DaemonSet centralizes but adds a hop. No strong prior; benchmark in Phase 2.
...
9. **Out-of-band Paymob reconciliation job cadence.** Hourly? Daily? Real-time stream? Trades off detection lag against API quota. Needs Paymob API documentation review.
...
- One namespace (`ahoy`), one workload (backend pod), one Service, one Ingress.
...
- A Helm chart for the backend, in a config repo (`ahoy-config`).
- GitHub Actions workflow that builds and pushes the image (unsigned, for now).

**Exit checkpoint:** `curl https://staging.ahoy.example/api/health/ready` returns 200, and the underlying request shows up in Grafana within 30 seconds.
...
- Two-repo pattern wired: CI in `ahoy-app` opens a PR in `ahoy-config` bumping image digest.
...
- Storefront, dashboard, and Celery workloads added.
...
- Apps emit traces with `trace_id` in logs.
...
- HPA configured (CPU + custom Celery queue-depth metric).
...
**Exit checkpoint:** a Chaos Mesh experiment kills the primary backend pod during peak-load simulation. The system recovers within the MTTR target with no human intervention beyond the runbook check. The experiment's postmortem is committed.
